...
- assumed you have setup SMO O2 endpoint for registration, e.g. http://<smo ip:port>/smo/o2/ocloud-registry/v1
- O2 service will post the O-Cloud registration data to that SMO O2 endpoint

::

  curl -k -X POST http(s)://<OAM IP>:30205/provision/smo-endpoint/v1 -d '{"smo-o2-endpoint": "http://<smo ip:port>/smo/o2/ocloud-registry/v1"}'
