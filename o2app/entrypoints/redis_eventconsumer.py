...
def main():
    ...
    pubsub.subscribe('NotifySmoChanged')
...
def handle_dms_changed(m, bus):
    ...
    if channel == "NfDeploymentStateChanged":
        ...
        bus.handle(cmd)
    if channel == 'NotifySmoChanged':
        datastr = m['data']
        data = json.loads(datastr)
        logger.info('NotifySmoChanged with cmd:{}'.format(data))
        cmd = imscmd.PubMessage2SMO(data=Message2SMO(
            id=data['id'],
            eventtype=data['notificationEventType'], ref=data['ref']))
        bus.handle(cmd)
    else:
