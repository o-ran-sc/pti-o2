...
from o2ims.domain.resource_type import ResourceTypeEnum, InvalidOcloudState
...
def update_ocloud(
    ...
    with uow:
        ...
        if oclouds and len(oclouds) > 1:
            ...
            entry = Ocloud(stxobj.id, stxobj.name, imsendpoint,
...
def is_outdated(ocloud: Ocloud, stxobj: StxGenericModel) -> bool:
    if stxobj.updatetime:
        return Ocloud.updatetime < stxobj.updatetime
    # logger.warning("hash1: " + self.hash + " vs hash2: " + stxobj.hash)
    else:
        return ocloud.hash != stxobj.hash
