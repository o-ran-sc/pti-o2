...
def create_by(stxobj: StxGenericModel, parent: Resource, resourcetype_id: str)\
    ...
    # items = [f"{k}:{content.get(k, '')}" for k in selected_keys]
    # description = ";".join(items)
    filtered = dict(filter(lambda item: item[0] in selected_keys, content.items()))
    extensions = json.dumps(filtered)
    description = f"name: {filtered.get('name')}, class: {filtered.get('pclass')}, vendor: {filtered.get('psvendor')}"
