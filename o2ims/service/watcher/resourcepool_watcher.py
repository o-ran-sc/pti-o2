...
class ResourcePoolWatcher(BaseWatcher):
    ...
    def _prune_stale_resourcepools_and_resources(self, ocloudid):
        """Prune DB resource pools that don't exist in the
            ...
            with self._bus.uow as uow:
                ...
                for pool_id in stale_ids:
                    ...
                    resources_del_result = uow.session.execute(
                        'DELETE FROM "resource" WHERE "resourcePoolId" = '
                        ':pool_id',
                        dict(pool_id=pool_id)
                    )
                    resources_deleted = getattr(
                        resources_del_result, 'rowcount', None
                    )

                    # 3) Delete the resource pool
                    uow.session.execute(
                        'DELETE FROM "resourcePool" WHERE '
                    ...
                    logger.info(
                        f'Pruned resource pool {pool_id} and related '
                        f'resources/alarms'
                    )
