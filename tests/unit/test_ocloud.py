...
def test_view_configurations(mock_uow):
    session, uow = mock_uow

    configuration_id1 = str(uuid.uuid4())
    conf1 = MagicMock()
    conf1.serialize.return_value = {
        "configurationId": configuration_id1,
    }
    session.return_value.query.return_value = [conf1]

    configuration_list = ocloud_view.configurations(uow)
    assert str(configuration_list[0].get(
        "configurationId")) == configuration_id1


def test_view_configuration_one(mock_uow):
    session, uow = mock_uow
    ...
    # Query return None
    configuration_res = ocloud_view.configuration_one(
...
def test_flask_get_list(mock_flask_uow):
    ...
    with app.test_client() as client:
        ...
        resp = client.get(apibase+"/configurations")
        assert resp.get_data() == b'[]\n'
...
def test_flask_get_one(mock_flask_uow):
    ...
    with app.test_client() as client:
        ...
        configuration_id1 = str(uuid.uuid4())
        resp = client.get(apibase+"/configurations/"+configuration_id1)
        assert resp.status_code == 404
...
def test_flask_post(mock_flask_uow):
    ...
    with app.test_client() as client:
        ...
        resp = client.post(apibase+'/configurations', json={
...
def test_flask_delete(mock_flask_uow):
    ...
    with app.test_client() as client:
        ...
        configuration_id1 = str(uuid.uuid4())
        resp = client.delete(apibase+"/configurations/"+configuration_id1)
        assert resp.status_code == 204
...
def test_flask_not_allowed(mock_flask_uow):
    ...
    with app.test_client() as client:
        ...
        # Testing configurations not support method
        ##########################
        uri = apibase + "/configurations"
        resp = client.put(uri)
        assert resp.status == '405 METHOD NOT ALLOWED'
        resp = client.patch(uri)
        assert resp.status == '405 METHOD NOT ALLOWED'
        ...
        subscription_id1 = str(uuid.uuid4())
        uri = apibase + "/configurations/" + subscription_id1
        resp = client.post(uri)
        assert resp.status == '405 METHOD NOT ALLOWED'
        resp = client.put(uri)
        assert resp.status == '405 METHOD NOT ALLOWED'
